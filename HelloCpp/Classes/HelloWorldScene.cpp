...
*/
   ...
   //scene
    ...
    maskedSceneSprite->init(backGroundSprite->getTexture(),"glow.png");
    ...
    //particle
    pParticle=CCParticleSystemQuad::create("huoshantishidian001.plist");
    pParticle->setPosition(ccp(winSize.width/2, winSize.height/2));
 ...
 /*
	...
	//slider2
    {
		...
		slider->setAnchorPoint(ccp(0.5f, 0.5f));
		slider->setMinimumValue(0.0f); // Sets the min value of range
		slider->setMaximumValue(maskedSceneSprite->maxBrightness*2); // Sets the max value of range
		slider->setValue(maskedSceneSprite->maxBrightness);
    ...
    //slider3
    {
		...
		slider->setAnchorPoint(ccp(0.5f, 0.5f));
		slider->setMinimumValue(0.0f); // Sets the min value of range
		slider->setMaximumValue(maskedSceneSprite->minBrightness*2); // Sets the max value of range
		slider->setValue(maskedSceneSprite->minBrightness);
    ...
    //slider4
    {
		...
		slider->setAnchorPoint(ccp(0.5f, 0.5f));
		slider->setMinimumValue(0.0f); // Sets the min value of range
		slider->setMaximumValue(maskedSceneSprite->wickR_div_r*2); // Sets the max value of range
		slider->setValue(maskedSceneSprite->wickR_div_r);
		slider->setPosition(m_pSliderCtl3->getPosition()+ccp(0,-m_pSliderCtl3->getContentSize().height));
		...
		uiRoot->addChild(slider,100);
		//title
		CCLabelTTF* pLabel = CCLabelTTF::create("wick R percentage ", "Arial", 30);
		pLabel->setPosition(ccp(slider->getPositionX()-slider->getContentSize().width/2-pLabel->getContentSize().width/2, slider->getPositionY()));
        uiRoot->addChild(pLabel, 1);
	}
    //slider5
    {
		...
		slider->setMaximumValue(maskedSceneSprite->wickBrightness*2); // Sets the max value of range
		slider->setValue(maskedSceneSprite->wickBrightness);
		slider->setPosition(m_pSliderCtl4->getPosition()+ccp(0,-m_pSliderCtl4->getContentSize().height));
		...
		uiRoot->addChild(slider,100);
		//title
		CCLabelTTF* pLabel = CCLabelTTF::create("wick brightness ", "Arial", 30);
		pLabel->setPosition(ccp(slider->getPositionX()-slider->getContentSize().width/2-pLabel->getContentSize().width/2, slider->getPositionY()));
        uiRoot->addChild(pLabel, 1);
	}
...
void HelloWorld::sliderAction2(CCObject* sender, CCControlEvent controlEvent)
{
    ...
    float value=pSlider->getValue();
    CCLOG("slider value:%f",value);
    maskedSceneSprite->maxBrightness=value;
...
void HelloWorld::sliderAction3(CCObject* sender, CCControlEvent controlEvent)
{
    ...
    float value=pSlider->getValue();
    CCLOG("slider value:%f",value);
    maskedSceneSprite->minBrightness=value;
...
void HelloWorld::sliderAction4(CCObject* sender, CCControlEvent controlEvent)
{
    ...
    float value=pSlider->getValue();
    CCLOG("slider value:%f",value);
    maskedSceneSprite->wickR_div_r=value;
...
void HelloWorld::sliderAction5(CCObject* sender, CCControlEvent controlEvent)
{
    ...
    float value=pSlider->getValue();
    CCLOG("slider value:%f",value);
    maskedSceneSprite->wickBrightness=value;
...
void HelloWorld::ccTouchesEnded(CCSet* touches, CCEvent* event)
{
    ...
    for( it = touches->begin(); it != touches->end(); it++)
    {
        ...
        maskedSceneSprite->c=loc_GLSpace;
         pParticle->setPosition(loc_GLSpace);
